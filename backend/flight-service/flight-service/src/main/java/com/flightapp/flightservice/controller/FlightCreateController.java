...
public class FlightCreateController {
	...
	@Operation(summary = "Update flight", description = "Allows admin users to update flight details")
	@PutMapping("/{id}")
	public ResponseEntity<Long> updateFlight(
			@Parameter(description = "User role forwarded by API Gateway", example = "ADMIN") @RequestHeader(value = AppConstants.USER_ROLE_HEADER, required = false) String userRole,
			@PathVariable Long id, @Valid @RequestBody FlightCreateRequest request) {
		if (userRole == null || !AppConstants.ADMIN_ROLE.equalsIgnoreCase(userRole)) {
			return ResponseEntity.status(HttpStatus.FORBIDDEN).build();
		}
		Long flightId = flightCreateService.updateFlight(id, request);
	...
	public ResponseEntity<FlightResponse> getFlightById(@PathVariable Long id) {
		FlightResponse response = flightSearchService.getFlightById(id);
