...
public class AuthServiceImpl implements AuthService {
	...
	public CommonResponse<SignupResponse> register(SignupRequest request) {
		...
		SignupResponse signupResponse = SignupResponse.builder().userId(savedUser.getId()).email(savedUser.getEmail())
				.role(savedUser.getRole().name()).build();

		return CommonResponse.<SignupResponse>builder().success(true).message("User registered successfully")
	...
	public CommonResponse<LoginResponse> login(LoginRequest request) {
		...
		LoginResponse loginResponse = LoginResponse
				.builder().token(token).tokenType("Bearer").expiresIn(3600).role(user.getRole().name()).user(UserInfo
						.builder().userId(user.getId()).fullName(user.getFullName()).email(user.getEmail()).build())
				.build();
